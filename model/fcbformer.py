...
class FCB(nn.Module):
    ...
    def forward(self, x):
        ...
        h = x
        feats = []
        for idx, module in enumerate(self.enc_blocks):
            h = module(h)
            if idx % 3 == 2:
                feats.append(h)
            hs.append(h)
...
class TB(nn.Module):
    def __init__(self, out_feat=False):
        super().__init__()
...
class CB(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        self.middle_block = nn.Sequential(RB(ch, ch), RB(ch, ch))
        self.LE = nn.ModuleList([])
        for mults in min_channel_mults[-4:]:
            self.LE.append(
                nn.Sequential(
                    RB(min_level_channels * mults, out_channels),
                    RB(out_channels, out_channels)
                )
            )
    ...
    def forward(self, x):
        ...
        for idx, module in enumerate(self.enc_blocks):
            ...
            if idx % 3 == 2:
                feats.append(h)
        feats = feats[2:]
        for i, module in enumerate(self.LE):
            feats[i] = self.LE[i](feats[i])
        return feats
    
@register_model
def fcbformer(pretrained=False, **kwargs):
    model = FCBFormer(**kwargs)
    return model